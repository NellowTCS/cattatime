...
function CatCup() {
  ...
  const saveConfig = useCallback(
    (newConfig) => {
      electronAPI
        .writeUserData(newConfig)
        .catch((err) => console.error('Error writing config:', err))
    },
    [electronAPI]
  )
  ...
  const buyItem = (item) => {
    // has to be here bc currency is here
    ...
    <div className="background" style={{ backgroundColor: themeColor }}>
      ...
      <div className="cup-page-container">
          ...
          >
            {ownedItems?.map((item, i) => (
              <img
                className="cup-cat"
                src={item.src}
                style={equippedItem == item.id ? {} : { display: 'none' }}
                key={i}
              />
            ))}
        ...
        <div className="cup-info-container">
          ...
          <p>
            Your cup is currently at {Math.floor(hours / 60 / 60)} hours and{' '}
            {Math.floor((hours / 60) % 60)} minutes out. That&apos;s out of the{' '}
            {Math.floor(maxHours / 60 / 60)} hours needed to fill your next cup.
          </p>
          ...
          <button className="glass-light" onClick={cashIn} disabled={hours < maxHours}>
            cash it in
          </button>
